...
const Loader = () => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'80vh', fontSize:12, color:'#94a3b8', fontWeight:700 }}>
    불러오는 중...
  </div>
);

function Guard({ children }) {
  return localStorage.getItem('accessToken') ? children : <Navigate to="/admin/login" replace />;
}
function LoginGuard({ children }) {
  return localStorage.getItem('accessToken') ? <Navigate to="/admin/dashboard" replace /> : children;
}
    ...
    <Router>
      <Suspense fallback={<Loader />}>
        <Routes>
          ...
          <Route path="/admin/login" element={<LoginGuard><LoginPage /></LoginGuard>} />

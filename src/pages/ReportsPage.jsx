...
import { getStatsDaily, getStatsMonthly } from '@apis/statsApi';
...
const MONTHLY_DATA = Array.from({ length: 12 }, (_, i) => ({
  ...
  제주공항점: 400 + i * 10,
  get total() {
    return LOCATIONS.reduce((sum, l) => sum + this[l], 0);
  },
}));
...
export default function ReportsPage() {
  ...
  const [monthlyKioskNames, setMonthlyKioskNames] = useState([]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getStatsDaily(dateRange.start, dateRange.end);
      const data = res?.data ?? res;

      // 백엔드 응답: [{ date, byKiosk: [{kioskId, kioskName, count}], dailyTotal }]
      // 테이블 형식으로 변환
      const rows = (Array.isArray(data) ? data : []).map((item) => {
        const row = { date: item.date, total: item.dailyTotal };
  ...
  // useEffect로 마운트 시 자동 조회
  useEffect(() => {
    getStatsMonthly()
      .then((res) => {
        const data = res?.data ?? res;
        const rows = (Array.isArray(data) ? data : []).map((item) => {
          const row = { month: item.date, total: item.dailyTotal };
          (item.byKiosk ?? []).forEach((k) => {
            row[k.kioskName] = k.count;
          });
          return row;
        ...
        // 키오스크 이름 동적 추출
        if (rows.length > 0) {
          setMonthlyKioskNames(Object.keys(rows[0]).filter((k) => k !== 'month' && k !== 'total'));
        }
      })
      .catch(console.error);
  }, []);

  // 기존 LOCATIONS 상수 대신 데이터에서 동적으로 키오스크 목록 추출
  const kioskNames = useMemo(() => {
    if (dailyData.length === 0) return [];
    const keys = Object.keys(dailyData[0]).filter((k) => k !== 'date' && k !== 'total');
    return keys;
  }, [dailyData]);
            ...
            <table className={shared.table} style={{ minWidth: 700 }}>
              ...
              <tbody>
                {monthlyData.length === 0 ? (
                  <tr>
                    <td
                      colSpan={monthlyKioskNames.length + 2}
                      style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}
                    >
                      데이터가 없습니다.
                    </td>
                  </tr>
